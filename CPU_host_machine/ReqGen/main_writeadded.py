...
class ReqGen:
    def __init__(self, args):
        ...
        self.document_dir = args.document_dir
        self.server_chat_url = f"http://{self.host}:{self.port}/chat"
        self.server_insert_url = f"http://{self.host}:{self.port}/insert_document"
    ...
    def _load_documents(self):
        ...
        if self.document_dir:
            logging.info(f"문서 디렉토리 '{self.document_dir}'에서 문서를 로드합니다...")
            # jsonl format
            # {"id":"36475584_0","contents":"Fabrication of a Vitamin B12-Loaded Carbon Dot\/Mixed-Ligand Metal Organic Framework Encapsulated within the Gelatin Microsphere for pH Sensing and In Vitro Wound Healing Assessment."}
            # get 1M documents from the tail of the file
            try:
                with FileReadBackwards(self.document_dir, encoding='utf-8') as frb:
                    for line in frb:
                        if line.strip():
                ...
                logging.error(f"문서 로드 실패: {e}. 빈 문서 리스트로 대체합니다.")
                return []   
        else:
            logging.info("문서 디렉토리가 지정되지 않았습니다. 쓰기 요청 시 문서를 생성하지 않습니다.")
            return documents
    ...
    async def _run_client(self, session, client_id):
        ...
        read_ratio, write_ratio = map(int, self.read_write_ratio.split('_'))
        ...
        while True:
            # 읽기/쓰기 비율에 따른 요청 유형 결정
            operation_choice = random.randint(1, total_ratio)
            if operation_choice <= read_ratio:
                # 읽기 요청
                ...
                try:
                    async with session.post(self.server_chat_url, json={"client_id": client_id, "user_input": question}) as response:
                ...
                # 쓰기 요청
                document = self.documents.pop()
                try: 
                    async with session.post(self.server_insert_url, json={"client_id": 0, "document": document}) as response:
                        response.raise_for_status()
                        await response.text()
                except asyncio.TimeoutError:
                    continue
...
def main():
    ...
    parser.add_argument("--document-dir", type=str, default="./documents", 
                        help="문서 파일들이 저장된 디렉토리 경로 (쓰기 요청 시 사용)")
    # =====================
